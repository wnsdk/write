...
export default function PromptIntroModal({ isOpen, onClose, prompt }) {
    ...
    const fetchLatestArticle = () => {
        if (prompt != null) {
            authAxios.get(`/article?promptId=${prompt.promptId}&isEvaluated=false`).then((response) => {
                if (response.data.content.length > 0) {
                    setlatestArticle(response.data.content[0]);
                }
            });
        }
    };
    ...
    return (
        <div className={`${styles.modalOverlay} ${isOpen ? styles.show : styles.hide}`} onClick={onClose}>
            {isOpen && (
                <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
                    ...
                    <div className={styles.right}>
                        ...
                        <Button
                            onClick={() =>
                                navigate(`/writing`, {
                                    state: { prompt: prompt, latestArticle: latestArticle },
                                })
                            }
                            text="시작하기"
                            width={'100%'}
