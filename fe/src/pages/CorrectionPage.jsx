...
export default function CorrectionPage() {
    ...
    const { correctionResult, prompt, body } = location.state || {};

    const highlight = (originalText) => {
        return originalText.replace(/\*\*(.*?)\*\*/g, '<span style="color: red;">$1</span>');
    };

    return (
        <div>
            <h1>AI 첨삭 결과</h1>
            {Array.from({ length: correctionResult.originalTexts.length }).map((item, index) => (
                <div key={index} className={styles.correction}>
                    {prompt.mode === 'TRANSLATING' && <div>{correctionResult.koreanTexts[index]}</div>}
                    ...
                    {prompt.mode !== 'COPYING' && <div>{correctionResult.explanations[index]}</div>}
                </div>
            ))}
            <h3>종합 평가</h3>
